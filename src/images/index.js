let hasBlobConstructor = typeof (Blob) !== 'undefined' && (function () {
  try {
    return Boolean(new window.Blob())
  } catch (e) {
    return false
  }
}())
...
let hasArrayBufferViewSupport = hasBlobConstructor && typeof (Uint8Array) !== 'undefined' && (function () {
  try {
    return new window.Blob([new Uint8Array(100)]).size === 100
  } catch (e) {
    return false
  }
}())

let hasToBlobSupport = (typeof HTMLCanvasElement !== 'undefined' ? window.HTMLCanvasElement.prototype.toBlob : false)
...
export default class ImageTools {
  ...
  static _toBlob (canvas, type) {
    ...
    if (dataURIParts[0].indexOf('base64') >= 0) {
      ...
      byteString = window.atob(dataURIParts[1])
    } else {
